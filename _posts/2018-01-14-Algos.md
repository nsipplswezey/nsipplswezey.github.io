...
Throughout my VoltAGE blogging/dev journaling I'm learning that writing clearly about what I'm working on is a big value add.

My organizing the work in some form that makes sense to an outside reader, I find my communication gets better. Writing clearly builds my reflexes for reducing complexity, focusing specifically, and verifying understanding. Additionally it can take a good day of coding that doesn't result in a commit, and turn it into a tangible memorable output. It amounts to a sort of daily micro-sprint reflection. And it also serves as a back reference. I've found throughout VoltAGE that after taking time off to work on other things, having a clear reference and linnear narrative, can get me right back into building. Additionally, I can look at my work from a third person perspective, and understand where I might improve my workflow, or where my bottle-necks or misunderstandings are. 

Additionally, I've learned that developing a template or model that fits my technical writing, is really useful. In VoltAGE that's been "Goal", "Scope: What needs to be done?", and "Up and Running". Essentially this is motivation, action, and proof of action.

This series is going to explore applying that template to understanding, exploring and deepening my practical experiences with the ritual of building and rebuilding datastructures and algorithms. I'll take the same approach of building quickly while reflecting and improving iteratively.

Building and rebuilding datastructures and algorithms seems to me to be a cultural ritual of the software engineering community. I'd like to use this space to highlight, from experience, the positive values I'm noticing in that ritual.

This is a first draft. Immediate responses might be: "Algorithms and data structures aren't a cultural 'ritual'. They're just proof that you know what you're doing. That you understand the fundamentals. They're proof that you aren't an imposter."

Additionally I personally believe there are really good reason to push back on spiritualizing or ritualizing technology, as it might seem I'm doing here. In spiritualizing tech, we run the risk of losing touch with the reality that technology is a set of tools that can be leveraged to solve a problem.

A tech sprituality reminds me of past religious eras, where we start to get all of our answers from one place. And that in so doing, we might start getting answers that do a lot of harm. And we might lose sight of the fact that there are serious problems in the world that technology might be very very bad at solving, even with claims –often with some empirical support– of being our current best shot. 

Having said all that, if your spirituality is technological, that's fine too. Spirituality is personal.

I use the word ritual somewhat begrudgingly, because it's a powerful word with deep contextual roots. Like any powerful context dependent word, it benefits from consistent clarification. 

The sense of ritual I'm intending to use is something like the dictionary definition of "a solemn ceremony consisting of a series of actions performed according to a prescribed order."

That word "solemn" is interesting to me, because it's not my own. Nor is it a word I (or anyone I know actually) actually use to communicate. 

The sense of "solemn" that I'm intending is "characterized by dignified or serious formality, as proceedings; of a formal or ceremonious character:"

## Goal?

Explore applying a technical blogging template to understanding, exploring and deepening my practical experiences with the ritual of building and rebuilding datastructures and algorithms. 

Building quickly while reflecting and improving iteratively.
...
Oddly enough, I think the first-draft soft-rant introduction to this blog post is probably sufficient for a first quick effort towards teaching the goal. How about that.

Let's dig in. The first step in the algorithms and data structures ritual I'm observing is that the constraints of the problem are important, and often not obvious. The constraints need to be discovered, interpretated, organized and maintained.

Let's discover some constraints.

Wait, how do we go about discovering constraints? So far it seems to me to be a combination of "try something" and "ask questions". And I guess 3rd, "relax, don't freak out and perserve."

Try something: I could build a quick linked list in JS using functional instantiation patterns. Then write a function that recurisvely searches the list, storing known values in a hash, and checking if a known value re-occurs.

Ask question: Do I get to build my own linked list? No.

Oh. Ok.

Question: What languages can you use? C, C++, Java, Python.

Oh...

So there you have it. We're toast. I'm going to have to refresh all my C, C++, Java or Python on the fly –which means I'm going to make lots of mistakes– and this is going to take me forever and now I'm embarassed.

Relax, don't freak out and perserve.

Of those 4, I'll take python. Even though I've been writing Android Java for the last 2 weeks, and have been reading and writing a little C++, python is higher level, seems less verbose, and might be more familiar to my world of web application engineering.
...
Now I'm wondering... how can I test whether my recursion is messed up. It looks like the linked list its running on has 746 elements. It seems like python should be able to support recursive functional calls at least 746 deep.
...
Definitely something wrong with my recursion. `A.next` is a node, not a value. So we need to change
...
Then here's where it gets tough. It turns out that the way the problem is designed, it breaks on a recursive solution.

What?! Yup. And it takes an hour or two of trying things, before you just accept that not using recursion might be an implicit part of the specc.

So you refactor, and you learn to just use a `while` loop instead of recursive approaches, and move on!
...
Use a set instead of a dictionary, and when we drop our recursive approach, we can define our set in scope of our method. Everything else is just about the same.

A few insights. This foray into the cultural ritual of algorithms and data structure problems is less about knowing your data structures and algorithms, and more about your solution search process.

In this case, what I notice most clearly, is the discovery of the ambiguity of the constraints.

"How is the link list constructed?" is a great question.

And an even more useful question is "does our environment support recursion?"

Recursion –a standard approach to traversal of a data structure– just plain wouldn't work(at least it seems that way).

Furthermore, the answer someone gives you to the question, might not be consistent or legitimate. That's ok. That's part of the ritual.

That highlights the depth to which you might question your assumptions.

Another insight is that it's always worthwhile to start iteratively, and only use recursion if necessary, or if asked.
